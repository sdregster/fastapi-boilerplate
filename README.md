# Шаблон приложения FastAPI с аутентификацией и авторизацией

Этот проект представляет собой готовый шаблон для разработки масштабируемых веб-приложений на основе **FastAPI** с
полноценной системой аутентификации и авторизации. Проект включает модульную архитектуру, поддерживает гибкое
логирование с **loguru**, и взаимодействие с базой данных через **SQLAlchemy** с асинхронной поддержкой. Система
миграций **Alembic** упрощает работу со схемой базы данных.

## Стек технологий

- **Веб-фреймворк**: FastAPI
- **ORM**: SQLAlchemy с асинхронной поддержкой (asyncpg для PostgreSQL)
- **База данных**: PostgreSQL с поддержкой асинхронных соединений
- **Система миграций**: Alembic с поддержкой асинхронных соединений
- **Авторизация/Аутентификация**: JWT токены с конфигурируемым временем жизни, bcrypt для хеширования паролей, python-jose для работы с токенами
- **Конфигурация**: Pydantic Settings с поддержкой вложенных настроек через переменные окружения

## Зависимости проекта

- `fastapi[all]==0.115.0` - высокопроизводительный веб-фреймворк
- `pydantic==2.9.2` - валидация данных
- `uvicorn==0.31.0` - ASGI-сервер
- `jinja2==3.1.4` - шаблонизатор
- `SQLAlchemy==2.0.35` - ORM для работы с базами данных
- `asyncpg==0.29.0` - асинхронная поддержка PostgreSQL
- `alembic==1.13.3` - управление миграциями базы данных
- `bcrypt==4.0.1` и `passlib[bcrypt]==1.7.4` - хеширование паролей
- `python-jose==3.3.0` - работа с JWT токенами
- `loguru==0.7.2` - красивое и удобное логирование

## Структура проекта

Проект построен с учётом модульной архитектуры, что позволяет легко расширять приложение и упрощает его поддержку.
Каждый модуль отвечает за отдельные задачи, такие как авторизация или управление данными.

### Основная структура проекта

```
├── app/
│   ├── auth/                   # Модуль авторизации и аутентификации
│   │   ├── dao.py              # Data Access Object для работы с БД
│   │   ├── models.py           # Модели данных для авторизации
│   │   ├── router.py           # Роутеры FastAPI для маршрутизации
│   │   ├── schemas.py          # Схемы для валидации данных
│   │   └── utils.py            # Вспомогательные функции для авторизации
│   ├── dao/                    # Общие DAO для приложения
│   │   ├── mixins/             # Миксины для моделей базы данных
│   │   │   ├── __init__.py     # Инициализация модуля миксинов
│   │   │   ├── base_mixin.py   # Базовый миксин с общими методами
│   │   │   ├── id_mixin.py     # Миксин для добавления поля ID
│   │   │   └── timestamp_mixin.py # Миксин для временных меток
│   │   ├── database.py         # Подключение к базе данных и управление сессиями
│   │   └── base.py             # Базовый класс DAO для работы с БД
│   ├── dependencies            # Зависимости в проекте
│   │   ├── auth_dep.py         # Зависимости для авторизации
│   │   └── dao_dep.py          # Зависимости для сессий SQLAlchemy
│   ├── utils/                  # Утилиты приложения
│   │   ├── __init__.py         # Инициализация модуля утилит
│   │   └── case_converter.py   # Конвертер имен из CamelCase в snake_case
│   ├── migration/              # Миграции базы данных
│   │   ├── versions/           # Файлы миграций
│   │   ├── env.py              # Настройки среды для Alembic
│   │   ├── README              # Документация по миграциям
│   │   └── script.py.mako      # Шаблон для генерации миграций
│   ├── static/                 # Статические файлы приложения
│   │   └── .gitkeep            # Пустой файл для сохранения папки в Git
│   ├── config.py               # Конфигурация приложения
│   ├── exceptions.py           # Исключения для обработки ошибок
│   ├── main.py                 # Основной файл для запуска приложения
├── data/                       # Папка для хранения данных (опционально)
├── .env                        # Конфигурация окружения
├── alembic.ini                 # Конфигурация Alembic
├── README.md                   # Документация проекта
└── requirements.txt            # Зависимости проекта
```

## 🧪 Тестирование

Проект включает полный набор тестов с 100% покрытием кода, организованных по типам.

### Запуск тестов
```bash
# Все тесты
pytest

# С подробным выводом
pytest -v

# Только модульные тесты
pytest -m "unit"
# или
pytest tests/unit/

# Только интеграционные тесты
pytest -m "integration"
# или
pytest tests/integration/

# Только e2e тесты
pytest -m "e2e"
# или
pytest tests/e2e/

# С покрытием кода
pytest --cov=app
```

### Структура тестов
- **Модульные тесты** (`tests/unit/`) - тестируют отдельные компоненты изолированно
- **Интеграционные тесты** (`tests/integration/`) - тестируют взаимодействие компонентов
- **End-to-End тесты** (`tests/e2e/`) - тестируют полные сценарии работы приложения

### Маркеры тестов
- `@pytest.mark.unit` - модульные тесты
- `@pytest.mark.integration` - интеграционные тесты  
- `@pytest.mark.e2e` - end-to-end тесты (медленные, полные сценарии)

Подробная документация по тестированию: [tests/README.md](tests/README.md)

Обновленный раздел с подробным описанием основных модулей:

---

### Основные модули

#### **app/auth** - Модуль для аутентификации и авторизации

Модуль отвечает за управление процессами аутентификации (входа пользователей) и авторизации (проверки доступа).  
Основные файлы:

- **`dao.py`**: Объект доступа к данным пользователей. Содержит методы для работы с базой данных (создание, обновление,
  поиск пользователей и т. д.).
- **`dependencies.py`**: Внедрение зависимостей, таких как проверка токенов и авторизация для защищённых маршрутов.
- **`models.py`**: Определяет ORM-модели данных для пользователей (например, таблица Users в базе данных).
- **`router.py`**: Роутер для маршрутизации запросов, связанных с аутентификацией. Определяет эндпоинты для входа,
  регистрации и проверки доступа.
- **`schemas.py`**: Определяет Pydantic-схемы для валидации входных данных и структуры ответов (например, формат данных
  для регистрации пользователей).
- **`utils.py`**: Вспомогательные функции для работы с токенами (создание, проверка JWT) и шифрование паролей.

---

#### **app/dao** - Базовый слой доступа к данным (Data Access Layer)

Модуль содержит абстракции для работы с базой данных. Используется для управления подключениями и реализации
CRUD-операций.

- **`base.py`**: Базовый класс DAO, предоставляющий общие методы для работы с базой данных, такие как добавление,
  обновление, удаление и поиск записей.
- **`database.py`**: Отвечает за подключение к базе данных, управление сессиями SQLAlchemy, а также создание
  асинхронного подключения (например, через `aiosqlite`).
- **`mixins/`**: Модуль с миксинами для расширения функциональности моделей базы данных:
  - **`base_mixin.py`**: Базовый миксин с общими методами для всех моделей (to_dict, __repr__).
  - **`id_mixin.py`**: Миксин для добавления первичного ключа ID с автоинкрементом.
  - **`timestamp_mixin.py`**: Миксин для автоматического добавления полей created_at и updated_at.

---

#### **app/migration** - Управление миграциями базы данных с Alembic

Модуль упрощает управление схемой базы данных и позволяет безопасно вносить изменения.

- **`versions/`**: Хранятся файлы миграций, автоматически создаваемые Alembic.
- **`env.py`**: Основной файл конфигурации для Alembic. Определяет подключение к базе данных и взаимодействие с ORM.
- **`script.py.mako`**: Шаблон для генерации новых файлов миграций.

---

#### **app/dependencies** - Управление зависимостями

Модуль содержит зависимости, которые используются в проекте.

- **`auth_dep.py`**: Зависимости, связанные с авторизацией пользователя в системе
- **`dao_dep.py`**: Зависимости, связанные с управлением сессией SQLAlchemy и с работой с дочерними классами BaseDAO

---

#### **app/utils** - Утилиты приложения

Модуль содержит вспомогательные функции и утилиты общего назначения.

- **`case_converter.py`**: Функции для преобразования строк из camelCase в snake_case, используемые для автоматической генерации имен таблиц на основе имен классов моделей.

---

#### **config.py** - Настройки и конфигурация приложения

Современная система конфигурации на основе Pydantic Settings с поддержкой вложенной структуры:

**Основные классы конфигурации**:
- **`RunConfig`**: настройки запуска (хост, порт)
- **`DatabaseConfig`**: конфигурация БД (URL, пулы соединений, настройки именования)
- **`JwtConfig`**: расширенные настройки JWT токенов
- **`Settings`**: главный класс, объединяющий все настройки

**Возможности**:
- Автоматическая загрузка из переменных окружения с префиксом `APP_CONFIG__`
- Поддержка вложенной структуры через разделитель `__`
- Приоритетная загрузка из файлов `.env` и `.env.template`
- Валидация типов данных и обязательных полей через Pydantic

**Примеры переменных окружения**:
- `APP_CONFIG__JWT__SECRET_KEY`: секретный ключ для JWT
- `APP_CONFIG__DB__URL`: URL подключения к базе данных
- `APP_CONFIG__JWT__ACCESS_TOKEN_EXPIRE_MINUTES`: время жизни access токена

---

#### **main.py** - Основной файл для запуска приложения

- **Инициализация приложения**: Настраивает FastAPI-приложение, включая параметры, такие как название, версия, и
  описание.
- **Подключение роутеров**: Регистрирует маршруты, определённые в модулях приложения, например:
    - `app.auth.router` для маршрутов авторизации.
    - Любые дополнительные модули (например, `app.users.router`).
- **Настройка зависимостей**: Внедряет глобальные зависимости, такие как подключение к базе данных или параметры
  конфигурации.
- **Настройка middleware**: Добавляет промежуточные слои для обработки запросов (например, CORS, сжатие, обработка
  ошибок).
- **Обработка ошибок**: Определяет глобальные обработчики исключений, чтобы возвращать понятные ответы при возникновении
  ошибок (например, 401 Unauthorized или 500 Internal Server Error).
- **Запуск сервера**: Используется для старта приложения с помощью ASGI-сервера (Uvicorn).

## Настройка аутентификации и авторизации

Система аутентификации построена на современных стандартах безопасности:

### **JWT Токены с расширенной безопасностью**
- **Access токены**: краткосрочные токены для доступа к API (по умолчанию 30 минут)
- **Refresh токены**: долгосрочные токены для обновления access токенов (по умолчанию 7 дней)

### **Поля безопасности в токенах**
- **`iss` (issuer)**: издатель токена для предотвращения подделки
- **`aud` (audience)**: целевая аудитория токена
- **`iat` (issued at)**: время создания токена
- **`exp` (expiration)**: время истечения токена
- **`sub` (subject)**: идентификатор пользователя

### **Конфигурируемые настройки**
Все параметры JWT настраиваются через переменные окружения:
- Время жизни токенов
- Алгоритмы подписи (поддержка HS256, RS256)
- Издатель и аудитория для дополнительной валидации
- Секретные ключи (с поддержкой ротации)

### **Безопасное хранение**
- Токены передаются через HTTP-only cookies
- Защита от XSS атак (httponly=True)
- Защита от CSRF атак (samesite="lax")
- Обязательное использование HTTPS в продакшене (secure=True)

## Запуск приложения

1. Клонируйте репозиторий:

   ```bash
   git clone https://github.com/Yakvenalex/FastApiWithAuthSample.git .
   ```

2. Установите зависимости:

   ```bash
   pip install -r requirements.txt
   ```

3. Создайте и настройте `.env` файл:

   ```env
   # База данных PostgreSQL
   APP_CONFIG__DB__URL=postgresql+asyncpg://postgres:password@localhost:5432/fastapi_auth
   APP_CONFIG__DB__ECHO=false
   
   # JWT токены (обязательные настройки)
   APP_CONFIG__JWT__SECRET_KEY=ваш_сгенерированный_секретный_ключ_здесь
   APP_CONFIG__JWT__ALGORITHM=HS256
   APP_CONFIG__JWT__ACCESS_TOKEN_EXPIRE_MINUTES=30
   APP_CONFIG__JWT__REFRESH_TOKEN_EXPIRE_DAYS=7
   APP_CONFIG__JWT__ISSUER=fastapi-auth-app
   APP_CONFIG__JWT__AUDIENCE=fastapi-auth-users
   ```

   **Примечание по безопасности**: Для генерации безопасного секретного ключа используйте команду:
   ```bash
   python -c "import secrets; print(secrets.token_urlsafe(32))"
   ```
   Это создаст криптографически стойкий ключ длиной 32 байта, закодированный в URL-safe формате.

   **Настройки JWT токенов**:
   - `ACCESS_TOKEN_EXPIRE_MINUTES`: время жизни access токена (по умолчанию 30 минут)
   - `REFRESH_TOKEN_EXPIRE_DAYS`: время жизни refresh токена (по умолчанию 7 дней)
   - `ISSUER` и `AUDIENCE`: дополнительные поля безопасности для валидации токенов

4. Запустите приложение с Uvicorn:

   ```bash
   uvicorn app.main:app --reload --port 8005
   ```

При необходимости замените port на нужный.

## Миграции базы данных

1. Инициализируйте Alembic:

   ```bash
   cd app
   alembic init -t async migration
   ```

   Затем переместите `alembic.ini` в корень проекта.

2. В `alembic.ini` установите `script_location` как `app/migration`.

3. Создайте миграцию:

   ```bash
   alembic revision --autogenerate -m "Initial migration"
   ```

4. Примените миграции:

   ```bash
   alembic upgrade head
   ```

## Лучшие практики

- Разделяйте функциональность приложения на модули для удобства тестирования и
  поддержки.
- Обрабатывайте ошибки с четкими ответами и HTTP-кодами.
- Проводите миграции с Alembic для управления схемой базы данных.
- Используйте переменные окружения для безопасного хранения конфиденциальных данных.
- **Безопасность**: Всегда используйте криптографически стойкие секретные ключи,
  сгенерированные с помощью модуля `secrets`. Никогда не используйте простые строки
  типа "supersecretkey" в продакшене.

---

Этот шаблон является мощной и удобной основой для разработки приложений на FastAPI с
поддержкой аутентификации, авторизации и структурированной архитектуры, готовой к
масштабированию.